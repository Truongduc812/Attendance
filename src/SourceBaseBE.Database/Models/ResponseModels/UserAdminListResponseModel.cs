...
{
    public class UserAdminListResponseModel
    {
        //[DisplayName("Id")]
        [JsonProperty("userid")]
        ...
        public UserAdminListResponseModel SetData(UserEntity? entity,List<DepartmentAdminEntity> departmentAdmins, List<DepartmentEntity> departments)
        {
            if (entity == null) throw new ArgumentNullException("Param Entity DashboardResponseModel Null");

            var department = departmentAdmins?.Where(x => x.UserId == entity?.Id)?.FirstOrDefault();
            var departmentName = departments.Where(x=>x.Id == department?.DepartmentId)?.FirstOrDefault();
            this.Name = entity?.Username;
            this.EmployeeCode = entity?.ItemEmployee?.EmployeeCode;
            this.Phone = entity?.PhoneNumber;
            this.Department = departmentName?.Name;
        ...
        {
            var properties = JsonPropertyHelper<UserAdminListResponseModel>.GetJsonPropertyNames();
            ...
            foreach (var data in datas)
            {
                if (properties.Contains(data.Key))
                {
                    ...
                    //* add flag_filterable
                    if (key == nameof(UserAdminListResponseModel.JobTitle).ToLower()
                        || (key == nameof(UserAdminListResponseModel.Role).ToLower()))

                        data.Filterable = true;
                    //* add flag_searchable
                    if (key == nameof(UserAdminListResponseModel.Name).ToLower()
                      || key == nameof(UserAdminListResponseModel.JobTitle).ToLower()
            || key == nameof(UserAdminListResponseModel.EmployeeCode).ToLower())
        ...
        public static IQueryable<UserEntity> PrepareWhereQuerySearch(IQueryable<UserEntity> query, SearchModel searchModel)
        {
            ...
            foreach (var search in dicSearch)
            {
                ...
                {
                    predicate = predicate.And(x => EF.Functions.Unaccent(x.Username.ToLower()).Contains(EF.Functions.Unaccent($"{searchVal}")));
                }

                if (key == "all")
                {
                    predicate = predicate.Or(x => EF.Functions.Unaccent(x.ItemEmployee.EmployeeCode.ToLower()).Contains(EF.Functions.Unaccent($"{searchVal}")));
                    predicate = predicate.Or(x => EF.Functions.Unaccent(x.ItemEmployee.JobTitle.Name.ToLower()).Contains(EF.Functions.Unaccent($"{searchVal}")));
                    predicate = predicate.Or(x => EF.Functions.Unaccent(x.Username.ToLower()).Contains(EF.Functions.Unaccent($"{searchVal}")));
                }
            }
            if (!string.IsNullOrWhiteSpace(searchKey))
            {
                predicate = predicate.Or(x => EF.Functions.Unaccent(x.ItemEmployee.JobTitle.Name).Contains(EF.Functions.Unaccent($"{searchKey}")));
                predicate = predicate.Or(x => EF.Functions.Unaccent(x.Username).Contains(EF.Functions.Unaccent($"{searchKey}")));
                predicate = predicate.Or(x => EF.Functions.Unaccent(x.ItemEmployee.EmployeeCode).Contains(EF.Functions.Unaccent($"{searchKey}")));
            }
        ...
        public static IQueryable<UserEntity> PrepareQuerySort(IQueryable<UserEntity> query, Dictionary<string, long> param)
        {
            ...
            foreach (var pa in param)
            {
                ...
                {
                    query = pa.Value == -1 ? query.OrderByDescending(x => x.Username) : query.OrderBy(x => x.Username);
                }
            }
            return query;
        }
