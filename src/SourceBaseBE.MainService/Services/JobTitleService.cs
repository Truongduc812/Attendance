...
{
	public class JobTitleService : BaseCRUDService<JobTitleEntity>
	{
		private UserRepository _authUserRepository;
		public JobTitleRepository _repositoryImp;

		/*[GEN-1]*/

		public JobTitleService(CommonDBContext dbContext, ILogger<JobTitleService> logger)
		  : base(dbContext, logger)
		{
			_repository = new JobTitleRepository(_dbContext);
			_repositoryImp = (JobTitleRepository)_repository;
			_authUserRepository = new UserRepository(_dbContext);

			/*[GEN-2]*/
		}
		public override JobTitleEntity GetById(long id, bool isDirect = false, bool isTracking = true)
		{
			var entity = _repository.GetById(id, isDirect, isTracking);
			var entityRS = (JobTitleEntity)_authUserRepository.FillTrackingUser(entity);
			return entityRS;
		}
		public override async Task<JobTitleEntity> GetByIdAsync(long id, bool isDirect = false, bool isTracking = true)
		{
			var entity = await _repository.GetByIdAsync(id, isDirect);
			var entityRS = (JobTitleEntity)_authUserRepository.FillTrackingUser(entity);
			return entityRS;
		}
		public override List<JobTitleEntity> GetList(PagingRequestModel pagingReq = null)
		{
			var list = _repository.GetList(pagingReq);
			var listRS = _authUserRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<JobTitleEntity>().ToList();
			return listRS;
		}
		public override List<Dictionary<string, object>> GetFormDataObjElement(JobTitleEntity entity)
		{
			string entityName = nameof(JobTitleEntity);
			var listRS = new List<Dictionary<string, object>>();
			List<object> objectList = null;
			var properties = typeof(JobTitleEntity).GetProperties();
		...
		/// <returns></returns>
		public override JobTitleEntity Upsert(JobTitleEntity entity, long? userId = null)
		{

			/*[GEN-3]*/
			var upsertedEntity = base.Upsert(entity/*[GEN-4]*/, userId);
			var entityRS = (JobTitleEntity)_authUserRepository.FillTrackingUser(upsertedEntity);
			return entityRS;
		}
		...
		/// <returns></returns>
		private List<long> GetListIdChildren(JobTitleEntity entity, string childEntity)
		{
			switch (childEntity)
			{
		...
		public IEnumerable<JobTitleEntity> InsertMulti(IEnumerable<JobTitleEntity> entity, long? userId = null)
		{

			/*[GEN-3]*/
			var upsertedEntity = ((JobTitleRepository)_repository).InsertMulti(entity/*[GEN-4]*/, userId);
			return upsertedEntity;
		}

		public List<JobTitleEntity> GetAll(MasterDataRequestModel masterDataRequest)
		{
			if (masterDataRequest.JobTitle == true)
			{
		...
		//public JobTitleEntity UpsertTestTransaction(JobTitleEntity entity, long? userId = null)
		//{
