...
{
    public class SystemSettingService : BaseCRUDService<SystemSettingEntity>
    {
        private UserRepository _userRepository;
        public SystemSettingRepository _repositoryImp;

        /*[GEN-1]*/

        public SystemSettingService(CommonDBContext dbContext, ILogger<SystemSettingService> logger)
          : base(dbContext, logger)
        {
            _repository = new SystemSettingRepository(_dbContext);
            _repositoryImp = (SystemSettingRepository)_repository;
            _userRepository = new UserRepository(_dbContext);

            /*[GEN-2]*/
        }
        public override SystemSettingEntity GetById(long id, bool isDirect = false, bool isTracking = true)
        {
            var entity = _repository.GetById(id, isDirect, isTracking);
            var entityRS = (SystemSettingEntity)_userRepository.FillTrackingUser(entity);
            return entityRS;
        }
        public override async Task<SystemSettingEntity> GetByIdAsync(long id, bool isDirect = false, bool isTracking = true)
        {
            var entity = await _repository.GetByIdAsync(id, isDirect);
            var entityRS = (SystemSettingEntity)_userRepository.FillTrackingUser(entity);
            return entityRS;
        }
        public override List<SystemSettingEntity> GetList(PagingRequestModel pagingReq = null)
        {
            var list = _repository.GetList(pagingReq);
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<SystemSettingEntity>().ToList();
            return listRS;
        }
        public override List<Dictionary<string, object>> GetFormDataObjElement(SystemSettingEntity entity)
        {
            string entityName = nameof(SystemSettingEntity);
            var listRS = new List<Dictionary<string, object>>();
            List<object> objectList = null;
            var properties = typeof(SystemSettingEntity).GetProperties().Where(x => x.Name == nameof(SystemSettingEntity.UpdateStateEmployeeInterval)
                                                                                  || x.Name == nameof(SystemSettingEntity.TimeSwitchInterval));

            //var properties = typeof(SystemSettingEntity).GetProperties() ;
          ...
          {"unit", formDataTypeAttr.Unit},
                        ...
                        case EnumFormDataType.textbox:
                            if (property.Name == nameof(SystemSettingEntity.UpdateStateEmployeeInterval))
                            {
                                listRS.Add(new Dictionary<string, object> {
          {"display_name", "INSIDE/OUTSIDE UPDATE TIME (Day)"},
          {"key", property.Name},
          {"value", property.GetValue(entity)},
          {"type", formDataTypeAttr.TypeName.ToStringValue()},
          ...
          {"unit", formDataTypeAttr.Unit},
                            ...
                            if (property.Name == nameof(SystemSettingEntity.TimeSwitchInterval))
                            {
                                listRS.Add(new Dictionary<string, object> {
          {"display_name", "EMPLOYEE PAGE SWITCH INTERVAL (Second)"},
          {"key", property.Name},
          {"value", property.GetValue(entity)},
          {"type", formDataTypeAttr.TypeName.ToStringValue()},
          {"placeholder", formDataTypeAttr.Placeholder},
        ...
        /// <returns></returns>
        public override SystemSettingEntity Upsert(SystemSettingEntity entity, long? userId = null)
        {

            /*[GEN-3]*/
            var upsertedEntity = ((SystemSettingRepository)_repository).Upsert(entity/*[GEN-4]*/, userId);
            var entityRS = (SystemSettingEntity)_userRepository.FillTrackingUser(upsertedEntity);
            return entityRS;
        }
        ...
        /// <returns></returns>
        private List<long> GetListIdChildren(SystemSettingEntity entity, string childEntity)
        {
            switch (childEntity)
            {
        ...
        //public SystemSettingEntity UpsertTestTransaction(SystemSettingEntity entity, long? userId = null)
        //{
