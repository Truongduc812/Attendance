...
{
  public class OrganizationService : BaseCRUDService<OrganizationEntity>
  {
    private UserRepository _authUserRepository;
    public OrganizationRepository _repositoryImp;
    
/*[GEN-1]*/

    public OrganizationService(CommonDBContext dbContext, ILogger<OrganizationService> logger)
      : base(dbContext, logger) 
    {
      _repository = new OrganizationRepository(_dbContext);
      _repositoryImp = (OrganizationRepository)_repository;
      _authUserRepository = new UserRepository(_dbContext);
      
/*[GEN-2]*/
    }
    public override OrganizationEntity GetById(long id, bool isDirect = false, bool isTracking = true)
    {
      var entity = _repository.GetById(id, isDirect, isTracking);
      var entityRS = (OrganizationEntity)_authUserRepository.FillTrackingUser(entity);
      return entityRS;
    }
    public override List<OrganizationEntity> GetList(PagingRequestModel pagingReq = null)
    {
      var list = _repository.GetList(pagingReq);
      var listRS = _authUserRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<OrganizationEntity>().ToList();
      return listRS;
    }
    public override List<Dictionary<string, object>> GetFormDataObjElement(OrganizationEntity entity)
    {
      string entityName = nameof(OrganizationEntity);
      var listRS = new List<Dictionary<string, object>>();
      List<object> objectList = null;
      var properties = typeof(OrganizationEntity).GetProperties();
    ...
    /// <returns></returns>
    public override OrganizationEntity Upsert(OrganizationEntity entity, long? userId = null)
    {
      
/*[GEN-3]*/
      var upsertedEntity = ((OrganizationRepository)_repository).Upsert(entity/*[GEN-4]*/, userId);
      var entityRS = (OrganizationEntity)_authUserRepository.FillTrackingUser(upsertedEntity);
      return entityRS;
    }
    ...
    /// <returns></returns>
    private List<long> GetListIdChildren(OrganizationEntity entity, string childEntity)
    {
      switch (childEntity)
      {
...
/*[GEN-6]*/
    ...
    //public OrganizationEntity UpsertTestTransaction(OrganizationEntity entity, long? userId = null)
    //{
