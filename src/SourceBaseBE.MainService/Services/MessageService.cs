...
{
  public class MessageService : BaseCRUDService<MessageEntity>
  {
    private UserRepository _userRepository;
    public MessageRepository _repositoryImp;
    
/*[GEN-1]*/

    public MessageService(CommonDBContext dbContext, ILogger<MessageService> logger)
      : base(dbContext, logger)
    {
      _repository = new MessageRepository(_dbContext);
      _repositoryImp = (MessageRepository)_repository;
      _userRepository = new UserRepository(_dbContext);
      
/*[GEN-2]*/
    }
    public override MessageEntity GetById(long id, bool isDirect = false, bool isTracking = true)
    {
      var entity = _repository.GetById(id, isDirect, isTracking);
      var entityRS = (MessageEntity)_userRepository.FillTrackingUser(entity);
      return entityRS;
    }
    public override async Task<MessageEntity> GetByIdAsync(long id, bool isDirect = false, bool isTracking = true)
    {
      var entity = await _repository.GetByIdAsync(id, isDirect);
      var entityRS = (MessageEntity)_userRepository.FillTrackingUser(entity);
      return entityRS;
    }
    public override List<MessageEntity> GetList(PagingRequestModel pagingReq = null)
    {
      var list = _repository.GetList(pagingReq);
      var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<MessageEntity>().ToList();
      return listRS;
    }
    public List<MessageEntity> GetListByUser(long userId, PagingRequestModel pagingReq = null)
    {
      var list = _repositoryImp.GetListByUser(userId, pagingReq);
      var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<MessageEntity>().ToList();
      return listRS;
    }
    public long GetTotalRecordByUserId(long userId)
    {
      var totalRecord = _repositoryImp.GetTotalRecordByUserId(userId);
      return totalRecord;
    }
    public override List<Dictionary<string, object>> GetFormDataObjElement(MessageEntity entity)
    {
      string entityName = nameof(MessageEntity);
      var listRS = new List<Dictionary<string, object>>();
      List<object> objectList = null;
      var properties = typeof(MessageEntity).GetProperties();
    ...
    /// <returns></returns>
    public override MessageEntity Upsert(MessageEntity entity, long? userId = null)
    {
      
/*[GEN-3]*/
      var upsertedEntity = ((MessageRepository)_repository).Upsert(entity/*[GEN-4]*/, userId);
      var entityRS = (MessageEntity)_userRepository.FillTrackingUser(upsertedEntity);
      return entityRS;
    }
    ...
    /// <returns></returns>
    private List<long> GetListIdChildren(MessageEntity entity, string childEntity)
    {
      switch (childEntity)
      {
...
/*[GEN-6]*/
    ...
    //public MessageEntity UpsertTestTransaction(MessageEntity entity, long? userId = null)
    //{
