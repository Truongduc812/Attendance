...
{
	public class DeviceService : BaseCRUDService<DeviceEntity>
	{
		private UserRepository _authUserRepository;
		public DeviceRepository _repositoryImp;

		/*[GEN-1]*/

		public DeviceService(CommonDBContext dbContext, ILogger<DeviceService> logger)
      : base(dbContext, logger)
    {
			_repository = new DeviceRepository(_dbContext);
			_repositoryImp = (DeviceRepository)_repository;
			_authUserRepository = new UserRepository(_dbContext);

			/*[GEN-2]*/
		}
		public override DeviceEntity GetById(long id, bool isDirect = false, bool isTracking = true)
		{
		...
		{
			var list = _repositoryImp.GetList(pagingReq, isHardGet);
			var listRS = _authUserRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<DeviceEntity>().ToList();
			return listRS;
		}
		public override List<Dictionary<string, object>> GetFormDataObjElement(DeviceEntity entity)
		{
			string entityName = nameof(DeviceEntity);
			var listRS = new List<Dictionary<string, object>>();
			List<object> objectList = null;
			var properties = typeof(DeviceEntity).GetProperties();
		...
		/// <returns></returns>
		public override DeviceEntity Upsert(DeviceEntity entity, long? userId = null)
		{

			/*[GEN-3]*/
			var upsertedEntity = ((DeviceRepository)_repository).Upsert(entity/*[GEN-4]*/, userId);
			var entityRS = (DeviceEntity)_authUserRepository.FillTrackingUser(upsertedEntity);
			return entityRS;
		}
		...
		/// <returns></returns>
		private List<long> GetListIdChildren(DeviceEntity entity, string childEntity)
		{
			switch (childEntity)
			{
    ...
    //public DeviceEntity UpsertTestTransaction(DeviceEntity entity, long? userId = null)
    //{
