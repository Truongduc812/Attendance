...
{
  public class AuthGroupService : BaseCRUDService<AuthGroupEntity>
  {
    private UserRepository _authUserRepository;
    private AuthPermissionRepository _authPermissionRepository;
/*[GEN-1]*/

    public AuthGroupService(CommonDBContext dbContext, ILogger<AuthGroupService> logger)
      : base(dbContext, logger)
    {
      _repository = new AuthGroupRepository(_dbContext);
      _authUserRepository = new UserRepository(_dbContext);
      this._authPermissionRepository = new AuthPermissionRepository(this._dbContext);
/*[GEN-2]*/
    }
    public override AuthGroupEntity GetById(long id, bool isDirect = false, bool isTracking = true)
    {
      var entity = _repository.GetById(id, isDirect, isTracking);
      var entityRS = (AuthGroupEntity)_authUserRepository.FillTrackingUser(entity);
      return entityRS;
    }
    public override List<AuthGroupEntity> GetList(PagingRequestModel pagingReq = null)
    {
      var list = _repository.GetList(pagingReq);
      var listRS = _authUserRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<AuthGroupEntity>().ToList();
      return listRS;
    }
    public override List<Dictionary<string, object>> GetFormDataObjElement(AuthGroupEntity entity)
    {
      string entityName = nameof(AuthGroupEntity);
      var listRS = new List<Dictionary<string, object>>();
      List<object> objectList = null;
      var properties = typeof(AuthGroupEntity).GetProperties();
    ...
    /// <returns></returns>
    public override AuthGroupEntity Upsert(AuthGroupEntity entity, long? userId = null)
    {
...
/*[GEN-3]*/
      var upsertedEntity = ((AuthGroupRepository)_repository).Upsert(entity, authPermissionChildren, authUserChildren/*[GEN-4]*/, userId);
      var entityRS = (AuthGroupEntity)_authUserRepository.FillTrackingUser(upsertedEntity);
      return entityRS;
    }
    ...
    /// <returns></returns>
    private List<long> GetListIdChildren(AuthGroupEntity entity, string childEntity)
    {
      switch (childEntity)
      {
...
/*[GEN-6]*/
    ...
    //public AuthGroupEntity UpsertTestTransaction(AuthGroupEntity entity, long? userId = null)
    //{
