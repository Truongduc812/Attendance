...
{
	public class ParameterService : BaseCRUDService<ParameterEntity>
	{
		private UserRepository _authUserRepository;
		public ParameterRepository _repositoryImp;

		/*[GEN-1]*/

		public ParameterService(CommonDBContext dbContext, ILogger<ParameterService> logger)
			: base(dbContext, logger) 
		{
			_repository = new ParameterRepository(_dbContext);
			_repositoryImp = (ParameterRepository)_repository;
			_authUserRepository = new UserRepository(_dbContext);

			/*[GEN-2]*/
		}
		public override ParameterEntity GetById(long id, bool isDirect = false, bool isTracking = true)
		{
			var entity = _repository.GetById(id, isDirect, isTracking);
			var entityRS = (ParameterEntity)_authUserRepository.FillTrackingUser(entity);
			return entityRS;
		}
		public override async Task<ParameterEntity> GetByIdAsync(long id, bool isDirect = false, bool isTracking = true)
		{
			var entity = await _repository.GetByIdAsync(id, isDirect);
			var entityRS = (ParameterEntity)_authUserRepository.FillTrackingUser(entity);
			return entityRS;
		}
		public override List<ParameterEntity> GetList(PagingRequestModel pagingReq = null)
		{
			var list = _repository.GetList(pagingReq);
			var listRS = _authUserRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<ParameterEntity>().ToList();
			return listRS;
		}
		public List<ParameterEntity> GetListByDevice(ParameterRequestPagingModel pagingReq = null)
		{
			var list = _repositoryImp.GetListByDeviceId(pagingReq.DeviceId.GetValueOrDefault(), pagingReq);
			var listRS = _authUserRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<ParameterEntity>().ToList();
			return listRS;
		}
		public override List<Dictionary<string, object>> GetFormDataObjElement(ParameterEntity entity)
		{
			string entityName = nameof(ParameterEntity);
			var listRS = new List<Dictionary<string, object>>();
			List<object> objectList = null;
			var properties = typeof(ParameterEntity).GetProperties();
		...
		/// <returns></returns>
		public override ParameterEntity Upsert(ParameterEntity entity, long? userId = null)
		{

			/*[GEN-3]*/
			var upsertedEntity = ((ParameterRepository)_repository).Upsert(entity/*[GEN-4]*/, userId);
			var entityRS = (ParameterEntity)_authUserRepository.FillTrackingUser(upsertedEntity);
			return entityRS;
		}
		...
		/// <returns></returns>
		private List<long> GetListIdChildren(ParameterEntity entity, string childEntity)
		{
			switch (childEntity)
			{
		...
		//public ParameterEntity UpsertTestTransaction(ParameterEntity entity, long? userId = null)
		//{
